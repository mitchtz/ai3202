...
def calc_util(row_y,col_x,graph,util_dict,discount_rate):
	if graph[row_y][col_x] == 50:
		...
		if (row_y-1) < 0:
			...
			neighbor_util[0] = util_dict[row_y-1][col_x]
		#East
		#Check if is out of bounds
		if (col_x+1) >= len(graph[row_y]):
			neighbor_util[1] = 0
		else:
			neighbor_util[1] = util_dict[row_y][col_x+1]
		#South
		#Check if is out of bounds
		if (row_y+1) >= len(graph):
			neighbor_util[2] = 0
		else:
			neighbor_util[2] = util_dict[row_y+1][col_x]
		#West
		#Check if is out of bounds
		if (col_x-1) < 0:
			neighbor_util[3] = 0
		else:
			neighbor_util[3] = util_dict[row_y][col_x-1]
		...
		dir_util = []
		#North
		dir_util[0] = (0.8*neighbor_util[0])+(0.1*neighbor_util[1])+(0.1*neighbor_util[3])
		#East
		dir_util[1] = (0.8*neighbor_util[1])+(0.1*neighbor_util[2])+(0.1*neighbor_util[0])
		#South
		dir_util[2] = (0.8*neighbor_util[2])+(0.1*neighbor_util[3])+(0.1*neighbor_util[1])
		#West
		dir_util[3] = (0.8*neighbor_util[3])+(0.1*neighbor_util[2])+(0.1*neighbor_util[0])
...
def mdp(graph,discount,epsilon):
